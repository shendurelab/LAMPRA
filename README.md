...
The preprocessing script (preprocess.py) is first run, converting raw winsorized MPRA scores into a categorical feature table in which each construct is encoded as ten integer columns — the identity and orientation of its five inserts — alongside the target activity score (avg_Rep), together with a fixed train/validation/holdout split. Four model families are then trained on these features to predict construct expression: linear models with optional pairwise interactions (train_linear.py; OLS, Ridge, Lasso, ElasticNet) and tree ensembles (train_rfxgb.py; random forest and XGBoost), each hyperparameter-tuned across the data subsets. The best model from each family is scored on the held-out set using evaluate_holdout.py, and a replicated random-forest learning curve is generated by down-sampling the training data across permutations (downsample_replicates.py). Finally, results are collated across families and subsets into model-comparison tables using model_comparison_merge.py and model_comparison_v3.1.py.
